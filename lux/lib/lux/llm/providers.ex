defmodule Lux.LLM.Providers do
  ...
  @doc """
  Starts the provider registry and owns its ETS tables for the lifetime of the
  Lux supervision tree.
  """
  ...
  defp clear_tables do
    for table <- [@registry_table, @circuit_table, @cost_table, @cache_table],
        :ets.info(table) != :undefined do
  ...
  def select_provider(opts \\ []) do
    ...
    |> choose_provider(opts)
  end

  defp choose_provider(candidates, opts) do
    selected =
      cond do
        opts[:model] ->
          Enum.find(candidates, &model_match?(&1, opts[:model]))

        opts[:task_type] == :reasoning ->
          pick_by_keywords(candidates, ~w(opus sonnet gpt-4o gpt-4))

        opts[:task_type] == :fast ->
          pick_by_keywords(candidates, ~w(mini haiku turbo))

        opts[:task_type] == :cheap ->
          Enum.min_by(candidates, &cheapest_cost/1, fn -> nil end)
  ...
  def select_fallback_chain(opts \\ []) do
    ...
    candidates =
      Enum.filter(
        list_providers(),
        &(&1.enabled && has_caps?(&1, caps) && !circuit_open?(&1.name))
      )

    case choose_provider(candidates, opts) do
      nil -> []
      selected -> [selected | Enum.reject(candidates, &(&1.name == selected.name))]
    end
  end
    ...
    case :ets.lookup(@circuit_table, name) do
      [{^name, %{state: :open, last_failure: last_fail}}] ->
        if last_fail && System.monotonic_time(:millisecond) - last_fail > @reset_timeout do
          :ets.insert(
            @circuit_table,
            {name, %{state: :half_open, failures: 0, last_failure: nil}}
          )
    ...
    case :ets.lookup(@circuit_table, name) do
      [{^name, %{failures: f} = state}] ->
        ...
        :ets.insert(
          @circuit_table,
          {name,
           %{state: new_state, failures: f2, last_failure: System.monotonic_time(:millisecond)}}
        )

        if new_state == :open do
          Logger.warning(
            "[Lux.LLM.Providers] Circuit breaker opened for #{inspect(name)} after #{f2} failures"
          )
        end
  ...
  def get_stats(opts \\ []) do
    ...
    %{
      ...
      estimated_cost_usd: Enum.sum_by(entries, & &1.estimated_cost_usd),
        ...
        |> Enum.map(fn {k, v} ->
          {k,
           %{
             ...
             estimated_cost_usd: Enum.sum_by(v, & &1.estimated_cost_usd),
  ...
  defp estimate_cost(provider_name, model, prompt_tokens, completion_tokens) do
    case get_provider(provider_name) do
      {:ok, provider} ->
        input_rate = price_for(provider.cost_per_input_token, model)
        output_rate = price_for(provider.cost_per_output_token, model)
  ...
  defp price_for(prices, model) do
    Map.get(prices || %{}, model, Map.get(prices || %{}, :default, 0.0))
  end
  ...
  def cache_response(key, response, ttl \\ :timer.minutes(5)) do
    :ets.insert(
      @cache_table,
      {cache_hash(key),
       %{response: response, expires_at: System.monotonic_time(:millisecond) + ttl}}
    )
  ...
  defp try_chain([p | rest], prompt, tools, opts, cache_key) do
    ...
    case result do
      {:ok, response} ->
        ...
        if Map.get(opts, :cache, true), do: cache_response(cache_key, response)
        {:ok, response}
  ...
  defp usage_tokens(%{metadata: metadata}) when is_map(metadata) do
  ...
  defp usage_tokens(_response), do: {0, 0}

  defp ensure_init! do
    case Process.whereis(__MODULE__) do
      nil ->
        raise "#{inspect(__MODULE__)} is not running; start the :lux application first"
  ...
  defp has_caps?(_provider, []), do: true
